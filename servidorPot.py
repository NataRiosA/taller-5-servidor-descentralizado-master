...
# Clase socket servidor	Potencia
class miHandler(socketserver.BaseRequestHandler):
	...
	def handle(self):
		...
		self.potencia = str(pot(self.num1, self.num2))
		print("La multiplicacion es =", self.potencia)

		# Enviamos el resultado
		self.request.send(self.potencia.encode("UTF-8"))
...
class serverThread(threading.Thread):
	...
	def run(self):
		...
		server1 = socketserver.TCPServer((host2, lsPuerto[5]), miHandler)
	...
	def run(self):
		...
		while True:
			...
			if opcion == "5":
				...
				# Llamamos la funcion conexion con Servidor Final
				resultadoPot = pot(numero1, numero2)
				print("\nNumero 1 = "+ numero1 + ", Numero 2 = " + numero2 + ", Operacion = ", opcion)
